...
for i = nday-1:-1:1
    ...
    if any(isnan(qR(i, :)))
        assignin('base', 'ind', i);
        assignin('base', 'qRi', qR(i, :));
        assignin('base', 'qRiprev', qR(i+1, :));
        assignin('base', 'pRupi', pRup(i+1, :));
        assignin('base', 'integ', integ);
        assignin('base', 'pstate', pstate);
        error('Problem with NaNs');
    end

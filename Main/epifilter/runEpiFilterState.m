...
function [Rmed, Rlow, Rhigh, Rm, pR, pRup, pstate] = runEpiFilterState(Rgrid, nPts, eta, nday, pr0, Lam, Iday)

% Assumptions and notes
% - uses different state models
...
% Precompute state distributions (additive)
pstate = zeros(nPts, nPts);
for j = 1:nPts
    pstate(j, :) = normpdf(Rgrid(j), Rgrid, sqrt(Rgrid)*eta);
end

% % Precompute state distributions (multiplicative)
% pstate = zeros(nPts, nPts);
% for j = 1:nPts
%     pstate(j, :) = normpdf(Rgrid(j), Rgrid, sqrt(eta));
% end
...
for i = 2:nday
    ...
    % State equations for R with averaging
    if i > 2
        pRavg = mean(pR(i-1:-1:max(1, i-7), :));
        pRavg = pRavg/sum(pRavg);
    else
        pRavg = pR(i-1, :);
    end

...
figure;
subplot(2, 1, 1);
plot(Rgrid, p1, 'LineWidth', 2);
subplot(2, 1, 2);
plot(Rgrid, pall, 'LineWidth', 2);
...
%% Simulate P(R1 > 1) and P(R > 1) for many scenarios

% Case 1: fix weight and lambda so P(R1 > 1) fixed
w1_1 = 1/3; L1_1 = 20; 
% Delta to incidence on region 1, rest keep at R <= 1
del = -0.95:0.05:0.95; I1_1 = (1 + del)*L1_1; ndel = length(del);
% Obtain CDF values
[F1_a, F2all_a, F3all_a, Fstruc_a] = getFconv(w1_1, L1_1, del, Rgrid, dR);

% Case 2: change weight to larger value 
w1_2 = 1/4; L1_2 = L1_1;
% Obtain CDF values
[F1_b, F2all_b, F3all_b, Fstruc_b] = getFconv(w1_2, L1_2, del, Rgrid, dR);

% Case 3: change total infectiousness to larger value 
L1_3 = 50; w1_3 = w1_1;
% Obtain CDF values
[F1_c, F2all_c, F3all_c, Fstruc_c] = getFconv(w1_3, L1_3, del, Rgrid, dR);
...
hold on; 
xlim([0 3]); ylabel('P($\bar{R}_s | I_1^s$)', 'FontSize', 20);  
...
subplot(3, 2, 2);
plot(del, F1_a, 'k', 'LineWidth', 2);
hold on;
plot(del, F2all_a, 'b', 'LineWidth', 2);
plot(del, F3all_a, 'r', 'LineWidth', 2);
hold off; box off; grid off; set(gca, 'YGrid', 'on');
ylabel('P($R_s > 1 | I_1^s$)', 'FontSize', 20);  
ylim([0 1.05]);
subplot(3, 2, 4);
plot(del, F1_b, 'k', 'LineWidth', 2);
hold on;
plot(del, F2all_b, 'b', 'LineWidth', 2);
plot(del, F3all_b, 'r', 'LineWidth', 2);
hold off; box off; grid off; set(gca, 'YGrid', 'on');
ylabel('P($R_s > 1 | I_1^s$)', 'FontSize', 20);
ylim([0 1.05]);
subplot(3, 2, 6);
plot(del, F1_c, 'k', 'LineWidth', 2);
hold on;
plot(del, F2all_c, 'b', 'LineWidth', 2);
plot(del, F3all_c, 'r', 'LineWidth', 2);
hold off; box off; grid off; set(gca, 'YGrid', 'on');
ylabel('P($R_s > 1 | I_1^s$)', 'FontSize', 20);
